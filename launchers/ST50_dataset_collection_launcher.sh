            ...
            for expert_probs in "${expert_probs_options[@]}"; do
                for random_init in "${random_init_options[@]}"; do
                    for random_goal in "${random_goal_options[@]}"; do
                        ...
                        job_name="ST50_ds${dataset_size}_ep${expert_probs//,/_}_ri${random_init}_rg${random_goal}"
                        ...
                        qsub -N "$job_name" \
                            -v ENV_LIST="$env_list_escaped",DATASET_SIZE="$dataset_size",EXPERT_PROBS="$expert_probs_escaped",SAVE_PATH="$save_path",RANDOM_INIT="$random_init",RANDOM_GOAL="$random_goal",TASKS="$task_escaped" \
