...
import pickle
import argparse
from dmc import make

# Create data directory if it doesn't exist
os.makedirs('data', exist_ok=True)

def collect_dataset(env_names, expert_probs, dataset_size=int(1e6), checkpoint=[3e5], frame_stack=3, action_repeat=1):
    """
    Collects a dataset of trajectories from DMC environments with image observations.
    ...
    Args:
        ...
        checkpoint: Checkpoint to save datasets
    ...
    for expert_prob in expert_probs:
        ...
        for env_name in env_names:
            ...
            env = make(env_name, frame_stack=frame_stack, action_repeat=action_repeat, seed=42)
            ...
            action_shape = (dataset_size,) + env.action_spec().shape
            ...
            while transitions_collected < dataset_size:
                # Only random action (no expert policy for DMC)
                a = np.random.uniform(
                ...
                if len(checkpoint) > 0 and any(s <= transitions_collected for s in checkpoint):
                    ...
                    dataset_path = f"data/exp={int(expert_prob*100)}/{env_name}_fs{frame_stack}_ar{action_repeat}_exp={int(expert_prob*100)}_ds={transitions_collected}"
            ...
            # Save the dataset
            dataset_path = f"data/exp={int(expert_prob*100)}/{env_name}_fs{frame_stack}_ar{action_repeat}_exp={int(expert_prob*100)}__ds={transitions_collected}"
            with open(dataset_path, 'wb') as f:
