"""
python pretrain_taco_multi_task_episodes_multihead.py --dataset_config data_episodes/ST50/0.0 --save_path models/debug/ --total_steps 300_000_000 --checkpoint "1_000_000, 2_000_000, 20_000_000,200_000_000" --lr 5e-4
python pretrain_taco_multi_task_episodes_multihead.py --dataset_config exp_local/metaworld/taco_prova_push-v2/buffer --save_path models/debug/ --total_steps 300_000_000 --checkpoint "1_000_000, 2_000_000, 20_000_000,200_000_000" --lr 5e-4
python pretrain_taco_multi_task_episodes_multihead.py --dataset_config data_episodes/my_config --save_path models/debug/ --total_steps 300_000_000 --checkpoint "1_000_000, 2_000_000, 20_000_000,200_000_000" --lr 5e-4
...
if __name__ == "__main__":
    ...
    parser.add_argument('--wandb_project', type=str, default='taco-mt-pretrain-cdmc', help='WandB project name')
    ...
    # Print task mapping information and get number of tasks
    num_tasks = 1  # default
    if hasattr(train_dataloader, 'task_to_id'):
        print(f"Task mapping: {train_dataloader.task_to_id}")
        print(f"ID to task mapping: {train_dataloader.id_to_task}")
        ...
        for batch in train_dataloader:
            ...
            if (test_dataloader is not None and 
                ...
                for _ in range(args.eval_batches):
                    ...
                    # For test set evaluation, always use test mode (averaged predictions)
                    test_metrics = taco_agent.evaluate_taco(obs, action, action_seq, r_next_obs, reward, task_id, use_test_mode=True)
            ...
            obs, action, action_seq, reward, discount, next_obs, r_next_obs, task_id = utils.to_torch(
                batch, args.device)
            # Training update - you can now use task_id in your algorithm if needed
            metrics = taco_agent.update_taco(obs, action, action_seq, r_next_obs, reward, task_id)
