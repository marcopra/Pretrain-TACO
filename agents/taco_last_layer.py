...
class TACOAgent:
    ...
    def freeze_all_but_last_layer(self):
        """
        Congela tutti i componenti tranne l'ultimo layer di actor e critic.
        Reinizializza gli ultimi layer di actor e critic.
        """
        utils.ColorPrint.blue("🔒 Freezing all networks except last layers of actor and critic")
        
        # Congela completamente encoder, TACO e act_tok
        self.encoder.eval()
        self.TACO.eval()
        self.act_tok.eval()
        ...
        # Congela tutto il critic tranne l'ultimo layer
        self.critic.eval()
        for param in self.critic.parameters():
            param.requires_grad = False
            
        # Scongela solo gli ultimi layer di Q1 e Q2
        for param in self.critic.Q1[-1].parameters():  # Ultimo Linear layer
            param.requires_grad = True
        for param in self.critic.Q2[-1].parameters():  # Ultimo Linear layer
            param.requires_grad = True
            
        # Congela tutto l'actor tranne l'ultimo layer
        self.actor.eval()
        for param in self.actor.parameters():
            param.requires_grad = False
            
        # Scongela solo l'ultimo layer della policy
        for param in self.actor.policy[-1].parameters():  # Ultimo Linear layer
            param.requires_grad = True
        
        # Reinizializza gli ultimi layer
        self.critic.Q1[-1].apply(utils.weight_init)
        self.critic.Q2[-1].apply(utils.weight_init)
        self.actor.policy[-1].apply(utils.weight_init)
        
        # Crea un nuovo optimizer solo per i parametri non congelati
        trainable_params = []
        for param in self.critic.Q1[-1].parameters():
            trainable_params.append(param)
        for param in self.critic.Q2[-1].parameters():
            trainable_params.append(param)
        for param in self.actor.policy[-1].parameters():
            trainable_params.append(param)
            
        # Selezione dell'optimizer per i layer finali
        if self.optimizer_type == "adam":
            optimizer_class = torch.optim.Adam
        elif self.optimizer_type == "sgd":
            optimizer_class = torch.optim.SGD
        ...
        # Sostituisci gli optimizer esistenti
        self.critic_opt = optimizer_class([p for p in self.critic.Q1[-1].parameters()] + 
                                        [p for p in self.critic.Q2[-1].parameters()], lr=self.critic_opt.param_groups[0]['lr'])
        self.actor_opt = optimizer_class(self.actor.policy[-1].parameters(), lr=self.actor_opt.param_groups[0]['lr'])
        
        # Salva i fingerprint per verifica
        self._last_layer_fingerprints = {
            'encoder': self._get_model_fingerprint(self.encoder),
            'taco': self._get_model_fingerprint(self.TACO),
            'act_tok': self._get_model_fingerprint(self.act_tok),
            'critic_trunk': {name: param.data.clone() for name, param in self.critic.trunk.named_parameters()},
            'critic_q1_except_last': {name: param.data.clone() for name, param in self.critic.Q1[:-1].named_parameters()},
            'critic_q2_except_last': {name: param.data.clone() for name, param in self.critic.Q2[:-1].named_parameters()},
            'actor_trunk': {name: param.data.clone() for name, param in self.actor.trunk.named_parameters()},
            'actor_policy_except_last': {name: param.data.clone() for name, param in self.actor.policy[:-1].named_parameters()}
        }
        
        self.freeze_encoder = True
        self.last_layer_only = True
        
        utils.ColorPrint.green("✓ Successfully froze all networks except last layers")
        utils.ColorPrint.green("✓ Reinitialized last layers of actor and critic")

    def _check_last_layer_frozen(self):
        """Verifica che i layer congelati non siano stati modificati"""
        if not hasattr(self, '_last_layer_fingerprints'):
            raise ValueError("No last layer fingerprints found. Did you call freeze_all_but_last_layer()?")
        
        fingerprints = self._last_layer_fingerprints
        
        # Verifica encoder
        current_encoder = self._get_model_fingerprint(self.encoder)
        for param_name, stored_param in fingerprints['encoder'].items():
            current_param = current_encoder[param_name]
            assert torch.all(torch.eq(current_param, stored_param)), f"Encoder parameter {param_name} has changed when it should be frozen!"
        
        # Verifica TACO
        current_taco = self._get_model_fingerprint(self.TACO)
        for param_name, stored_param in fingerprints['taco'].items():
            current_param = current_taco[param_name]
            assert torch.all(torch.eq(current_param, stored_param)), f"TACO parameter {param_name} has changed when it should be frozen!"
        
        # Verifica act_tok
        current_act_tok = self._get_model_fingerprint(self.act_tok)
        for param_name, stored_param in fingerprints['act_tok'].items():
            current_param = current_act_tok[param_name]
            assert torch.all(torch.eq(current_param, stored_param)), f"act_tok parameter {param_name} has changed when it should be frozen!"
        
        # Verifica critic trunk
        current_critic_trunk = {name: param.data.clone() for name, param in self.critic.trunk.named_parameters()}
        for param_name, stored_param in fingerprints['critic_trunk'].items():
            current_param = current_critic_trunk[param_name]
            assert torch.all(torch.eq(current_param, stored_param)), f"Critic trunk parameter {param_name} has changed when it should be frozen!"
        
        # Verifica critic Q1 (escluso ultimo layer)
        current_critic_q1 = {name: param.data.clone() for name, param in self.critic.Q1[:-1].named_parameters()}
        for param_name, stored_param in fingerprints['critic_q1_except_last'].items():
            current_param = current_critic_q1[param_name]
            assert torch.all(torch.eq(current_param, stored_param)), f"Critic Q1 parameter {param_name} has changed when it should be frozen!"
        
        # Verifica critic Q2 (escluso ultimo layer)
        current_critic_q2 = {name: param.data.clone() for name, param in self.critic.Q2[:-1].named_parameters()}
        for param_name, stored_param in fingerprints['critic_q2_except_last'].items():
            current_param = current_critic_q2[param_name]
            assert torch.all(torch.eq(current_param, stored_param)), f"Critic Q2 parameter {param_name} has changed when it should be frozen!"
        
        # Verifica actor trunk
        current_actor_trunk = {name: param.data.clone() for name, param in self.actor.trunk.named_parameters()}
        for param_name, stored_param in fingerprints['actor_trunk'].items():
            current_param = current_actor_trunk[param_name]
            assert torch.all(torch.eq(current_param, stored_param)), f"Actor trunk parameter {param_name} has changed when it should be frozen!"
        
        # Verifica actor policy (escluso ultimo layer)
        current_actor_policy = {name: param.data.clone() for name, param in self.actor.policy[:-1].named_parameters()}
        for param_name, stored_param in fingerprints['actor_policy_except_last'].items():
            current_param = current_actor_policy[param_name]
            assert torch.all(torch.eq(current_param, stored_param)), f"Actor policy parameter {param_name} has changed when it should be frozen!"
            
    def train(self, training=True):
        self.training = training
        # Se stiamo usando last_layer_only, non cambiare il mode dei moduli congelati
        if hasattr(self, 'last_layer_only') and self.last_layer_only:
            # Solo gli ultimi layer devono essere in training mode
            if training:
                # Gli ultimi layer dell'actor e critic vengono messi in training mode
                self.critic.Q1[-1].train()
                self.critic.Q2[-1].train()
                self.actor.policy[-1].train()
        else:
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, step):
        ...
        # optimize encoder and critic - but not if we're using last_layer_only mode
        if not self.freeze_encoder and not (hasattr(self, 'last_layer_only') and self.last_layer_only):
    ...
    def update_taco(self, obs, action, action_seq, next_obs, reward):
        ...
        # Non fare update se l'encoder è congelato o se stiamo usando last_layer_only
        if not self.freeze_encoder and not (hasattr(self, 'last_layer_only') and self.last_layer_only):
    ...
    def update(self, replay_iter, step):
        ...
        # Verify that frozen models haven't been modified (if using last_layer_only mode)
        if hasattr(self, 'last_layer_only') and self.last_layer_only:
            self._check_last_layer_frozen()
