...
class TACO(nn.Module):
    ...
    def compute_logits_distributed(self, z_a, z_pos):
        """
        Sincronizza le features tra tutti i processi e calcola la contrastive loss globale.
        """
        # Raccogli tutte le features da tutti i processi
        world_size = dist.get_world_size() if dist.is_initialized() else 1
        if world_size == 1:
            return self.compute_logits(z_a, z_pos)

        # All-gather per z_a e z_pos
        z_a_list = [torch.zeros_like(z_a) for _ in range(world_size)]
        z_pos_list = [torch.zeros_like(z_pos) for _ in range(world_size)]
        dist.all_gather(z_a_list, z_a)
        dist.all_gather(z_pos_list, z_pos)
        z_a_all = torch.cat(z_a_list, dim=0)
        z_pos_all = torch.cat(z_pos_list, dim=0)

        # Calcola logits globali
        Wz = torch.matmul(self.W, z_pos_all.T)
...
class TACOAgent:
    def __init__(self, obs_shape, action_shape, device, lr, encoder_lr, feature_dim,
                 ...
                 freeze_encoder=False, no_taco=False, use_distributed=False, rank=0):
        ...
        # Wrappa i modelli con DDP se distribuito
        if self.use_distributed:
            from torch.nn.parallel import DistributedDataParallel as DDP
            if not self.freeze_encoder:
                self.encoder = DDP(self.encoder, device_ids=[self.rank])
                self.TACO = DDP(self.TACO, device_ids=[self.rank])
                if hasattr(self.act_tok, 'parameters') and any(p.requires_grad for p in self.act_tok.parameters()):
                    self.act_tok = DDP(self.act_tok, device_ids=[self.rank])
            self.actor = DDP(self.actor, device_ids=[self.rank])
            self.critic = DDP(self.critic, device_ids=[self.rank])
            self.critic_target = DDP(self.critic_target, device_ids=[self.rank])

        # Store original models for accessing non-DDP versions
        self._encoder = self.encoder
        self._TACO = self.TACO
        self._act_tok = self.act_tok
    ...
    def act(self, obs, step, eval_mode):
        ...
        dist_ = self.actor(obs, stddev)
        if eval_mode:
            action = dist_.mean
        else:
            action = dist_.sample(clip=None)
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, step):
        ...
        with torch.no_grad():
            stddev = utils.schedule(self.stddev_schedule, step)
            dist_ = self.actor(next_obs, stddev)
            next_action = dist_.sample(clip=self.stddev_clip)
        ...
        if not self.freeze_encoder and self.encoder_opt is not None:
    ...
    def update_actor(self, obs, step):
        ...
        stddev = utils.schedule(self.stddev_schedule, step)
        dist_ = self.actor(obs, stddev)
        action = dist_.sample(clip=self.stddev_clip)
        log_prob = dist_.log_prob(action).sum(-1, keepdim=True)
        Q1, Q2 = self.critic(obs, action, self.act_tok)
        Q = torch.min(Q1, Q2)
    ...
    def update_taco(self, obs, action, action_seq, next_obs, reward):
        ...
        z_a = self.TACO.module.encode(obs_anchor) if isinstance(self.TACO, nn.parallel.DistributedDataParallel) else self.TACO.encode(obs_anchor)
        z_pos = self.TACO.module.encode(obs_pos, ema=True) if isinstance(self.TACO, nn.parallel.DistributedDataParallel) else self.TACO.encode(obs_pos, ema=True)
        # Contrastive loss distribuita
        if self.curl:
            if self.use_distributed and dist.is_initialized():
                logits = self.TACO.module.compute_logits_distributed(z_a, z_pos)
            else:
                logits = self.TACO.compute_logits(z_a, z_pos)
            labels = torch.arange(logits.shape[0]).long().to(self.device)
            curl_loss = self.cross_entropy_loss(logits, labels)
        else:
            curl_loss = torch.tensor(0., device=self.device)
    ...
    def update(self, replay_iter, step):
        ...
        utils.soft_update_params(self.get_critic(), self.get_critic_target(),
