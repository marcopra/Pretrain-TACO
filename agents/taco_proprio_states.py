...
class Encoder(nn.Module):
    def __init__(self, obs_shape, repr_dim, hidden_dim=[1024,1024]):
        super().__init__()
        assert len(obs_shape) == 1, f"working with proprio states, obs_shape should be 1D, got {obs_shape}"

        self.encoder = utils.create_mlp(input_dim=obs_shape[0], output_dim=repr_dim, hidden_sizes=hidden_dim, output_activation="relu")
...
class TACOAgent:
    def __init__(self, obs_shape, action_shape, device, lr, encoder_lr, repr_dim, encoder_hidden_dim, feature_dim,
                 hidden_dim, critic_target_tau, num_expl_steps,
                 update_every_steps, stddev_schedule, stddev_clip, use_tb,
                 reward, multistep, latent_a_dim, pretrained_path=None, freeze_encoder=False, no_taco=False, optimizer_type="adam"):
        ...
        self.encoder = Encoder(obs_shape, repr_dim, encoder_hidden_dim).to(device)
    ...
    def update_taco(self, obs, action, action_seq, next_obs, reward):
        ...
        z_a = self.TACO.encode(obs.float())
        ...
        ### Compute TACO loss
        next_z = self.TACO.encode(next_obs.float(), ema=True)
        ...
        if not self.freeze_encoder:
            ...
            (taco_loss  + reward_loss).backward()
            self.taco_opt.step()
        if self.use_tb:
            metrics['reward_loss']  = reward_loss.item()
            metrics['taco_loss']  = taco_loss.item()
            metrics['total_loss'] = taco_loss.item() + reward_loss.item()
        return metrics
