...
case $model_exp in
    0)
        model_path="models/big_dataset/conv/taco_MT_MT49OODButtonPress.json_lr\=0.0005_ts\=182323200_len\=992250_curl_rew_best.pt"
        wandb_tag="0_BUTTONPRESS_1M"
        ;;
    33)
        # Throw an error
        echo "Error: Model experiment 33 is not supported."
        exit 1
        ;;
    66)
        echo "Error: Model experiment 66 is not supported."
        exit 1
        ;;
    99)
        echo "Error: Model experiment 99 is not supported."
        exit 1
        ;;
    *)
