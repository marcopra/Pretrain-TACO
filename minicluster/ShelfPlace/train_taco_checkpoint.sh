...
for i in $(seq 1 $num_runs); do
    ...
    python3 train_metaworld_checkpoint.py agent=taco_checkpoint exp_name=baseline${cuda_device}${SEED} seed=${SEED} env_name=shelf-place-v2 random_init=true random_goal=false wandb_tag=baseline num_train_frames=8000000 device=cuda:${cuda_device} agent.no_taco=${no_taco} batch_size=${batch_size} wandb_project="checkpoint" encoder_checkpoint_steps=[10000,50000,100000,2000000,4000000]
