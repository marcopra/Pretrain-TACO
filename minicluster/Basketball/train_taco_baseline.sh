...
for i in $(seq 1 $num_runs); do
    ...
    python3 train_metaworld.py exp_name=baseline${cuda_device} seed=1 env_name=basketball-v2 random_init=true random_goal=false wandb_tag=baseline num_train_frames=1100000 device=cuda:${cuda_device} agent.no_taco=${no_taco} agent.batch_size=${batch_size} wandb_project="taco_multihead_long"
