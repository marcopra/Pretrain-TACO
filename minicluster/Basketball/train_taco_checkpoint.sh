...
if [ $# -lt 1 ] || [ $# -gt 3 ]; then
    ...
    echo "batch_size: positive integer (default: 1024)"
    exit 1
fi
...
for i in $(seq 1 $num_runs); do
    ...
    python3 train_metaworld_checkpoint.py agent=taco_checkpoint exp_name=baseline${cuda_device}${SEED} seed=${SEED} env_name=basketball-v2 random_init=true random_goal=false wandb_tag=baseline num_train_frames=800000 device=cuda:${cuda_device} agent.no_taco=${no_taco} batch_size=${batch_size} wandb_project="checkpoint" encoder_checkpoint_steps=[100000,150000,250000,300000,400000]
