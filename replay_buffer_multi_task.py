...
def save_episode_with_task_id(episode, fn, task_id):
    """Save episode with task_id embedded in the data"""
    # Add task_id to episode data
    episode_with_task = dict(episode)
    # Create task_id array with same length as episode
    obs_key = next(iter(episode.keys()))
    episode_len = episode[obs_key].shape[0]
    episode_with_task['task_id'] = np.full((episode_len,), task_id, dtype=np.int32)
    
    with io.BytesIO() as bs:
        np.savez_compressed(bs, **episode_with_task)
        bs.seek(0)
        with fn.open('wb') as f:
            f.write(bs.read())
...
class ReplayBuffer(IterableDataset):
    ...
    def _sample(self):
        ...
        # Extract task_id if available, otherwise use -1 as default
        task_id = episode['task_id'][idx]
        if np.isscalar(task_id):
            task_id = np.array([task_id])
        
        return (obs, action, action_seq, reward, discount, next_obs, r_next_obs, task_id)
