...
from replay_buffer_multinode import ReplayBufferStorage, make_replay_loader
...
def setup_distributed(rank, world_size):
    ...
    os.environ['MASTER_ADDR'] = 'localhost' if world_size == 1 else os.environ.get('MASTER_ADDR', 'localhost')
...
def make_agent(obs_spec, action_spec, cfg, rank=0, world_size=1):
    cfg.obs_shape = obs_spec.shape
    cfg.action_shape = action_spec.shape
    # Passa use_distributed e rank a TACOAgent
    agent = hydra.utils.instantiate(cfg, use_distributed=(world_size > 1), rank=rank)
    return agent
...
class Workspace:
    def __init__(self, cfg, rank=0, world_size=1):
        ...
        utils.set_seed_everywhere(cfg.seed + rank)  # Different seed per process
        ...
        # Update cfg.agent.device to use the correct device
        print(f"Setting agent device to {self.device}")
        self.cfg.agent.device = str(self.device)
        self.agent = make_agent(obs_spec, action_spec, self.cfg.agent, rank=rank, world_size=world_size)
        ...
        if cfg.use_wandb and self.rank == 0:
            if cfg.wandb_id is not None and cfg.wandb_id != "none":
                ...
                # Create a safe config dict without problematic interpolations
                try:
                    safe_config = OmegaConf.to_container(cfg, resolve=True)
                except Exception as e:
                    print(f"Warning: Could not resolve full config for wandb: {e}")
                    # Create a minimal config dict with essential parameters
                    safe_config = {
                        'task': cfg.task,
                        ...
                        'seed': cfg.seed,
                        'use_distributed': getattr(cfg, 'use_distributed', False)
                    }
                ...
                wandb.init(
                    ...
                    tags=cfg.wandb_tag.split('_') if hasattr(cfg, 'wandb_tag') and cfg.wandb_tag and cfg.wandb_tag != "none" else None,
                    sync_tensorboard=True,
                    mode='online')
    ...
    def train(self):
        ...
        while train_until_step(self.global_step):
            if time_step.last():
                ...
                if self.rank == 0:  # Only record video on rank 0
            ...
            # try to evaluate (only on rank 0)
...
def run_distributed_training(rank, world_size, cfg):
    """Function to run distributed training on each process"""
    workspace = Workspace(cfg, rank, world_size)
    if rank == 0:  # Only load snapshot on rank 0
        snapshot = Path.cwd() / 'snapshot.pt'
        if snapshot.exists():
            print(f'resuming: {snapshot}')
            workspace.load_snapshot()
    
    # Broadcast loaded state to all processes if needed
    if world_size > 1:
        dist.barrier()
...
def main(cfg):
    ...
    world_size = torch.cuda.device_count() if cfg.use_distributed else 1
    
    if world_size > 1 and cfg.use_distributed:
